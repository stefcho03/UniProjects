...
void mainISR(void)
{
     ...
     */
    magni1.update(&Sensor1ISR,&Sensor2ISR,&Sensor3ISR,&Sensor4ISR,&Sensor5ISR);
...
void main(void)
{
    ...
    debugTimer.init(&magniSys, CFG_US_TIMER_BASE, &debugISR, CFG_US_TIMER_FREQ);

    //Initializes and starts the Magni
    magni1.init(&magniSys, &ISRLeft, &ISRRight,&Sensor1ISR,&Sensor2ISR,&Sensor3ISR,&Sensor4ISR,&Sensor5ISR);
    mainTimer.start();
    while(1)
    {
