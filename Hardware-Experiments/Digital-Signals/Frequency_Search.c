...
// the 7th and the 8th led shows us whether we are between 0.5-1 kHz or 1.5-2 kHz
if(position <= 20 && position >= 15){
GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_7,0xff);
}
if(position<= 10 && position >= 5){
GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_6,0xff);
}
// the position multiplied with 100 is our frequency
// one led covers two frequencies
if(position== 20 || position == 10){
GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_5,0xff);
}
if(position==19 || position == 9){
GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_4,0xff);
}
if(position== 18 || position == 8){
